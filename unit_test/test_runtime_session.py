...
class RuntimeSessionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_message_generates_design_without_orchestrator(self) -> None:
        runtime = CreativeClawRuntime()
        inbound = InboundMessage(
            channel="cli",
            sender_id="cli-user",
...
""",
            ...
            async def _fake_dispatch(**kwargs):
                tool_result = {
                    ...
                    "status": "success",
                    "message": f"Generated html code at {output_record['path']}.",
                    "output_text": f"Generated html code at {output_record['path']}.",
                ...
                return SimpleNamespace(
                    ...
                    state_delta={
                        ...
                        "generated": [output_record],
                        "new_files": [output_record],
                        "files_history": [[output_record]],
                        "summary_history": [f"CodeGenerationExpert: {tool_result['message']}"],
                        "message_history": [tool_result["message"]],
                        "text_history": [tool_result["output_text"]],
                    },
                )

            with (
                patch("src.runtime.workflow_service.Orchestrator") as mocked_orchestrator,
                patch("src.runtime.workflow_service.dispatch_expert_direct", new=_fake_dispatch),
            ):
                events = [event async for event in runtime.run_message(inbound)]
            ...
            self.assertEqual(events[0].event_type, "status")
            self.assertEqual(events[-1].event_type, "final")
            self.assertIn("设计产物已生成", events[-1].text)
            self.assertIn(str(output_record["path"]), events[-1].text)
            ...
            self.assertEqual(session.state["workflow_status"], "finished")
            self.assertEqual(session.state["final_file_paths"], [output_record["path"]])
            self.assertEqual(session.state["design_product_result"]["status"], "success")
            self.assertEqual(session.state["design_product_result"]["design_validation"][0]["status"], "pass")
