...
async def generate_code_artifact(
    ...
    *,
    ...
    try:
        ...
        generated_text = await _collect_generated_code_text(
            llm,
            run_context,
...
async def _collect_generated_code_text(
    llm: LlmAgent,
    run_context: Any,
    *,
    timeout_seconds: float,
) -> str:
    """Collect generated code text from an ADK LLM run.
    ...
    if latest_complete_text:
        ...
        return latest_complete_text
    if partial_chunks:
        logger.warning("code artifact generation used partial text fallback")
        return strip_code_fence("".join(partial_chunks))
    return ""


def _extract_event_text(event: Any) -> str:
    """Return concatenated text parts from one ADK event."""
    content = getattr(event, "content", None)
    parts = getattr(content, "parts", None) or []
    texts = [str(text) for part in parts if (text := getattr(part, "text", None))]
