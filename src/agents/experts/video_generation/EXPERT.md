...
parameter_examples = [
  "{'prompt': 'make a cinematic cat video', 'prompt_rewrite': 'auto|off'(optional, agent-side), 'provider': 'seedance|veo|kling|dashscope'(optional), 'mode': 'prompt'(optional), 'aspect_ratio': '16:9|4:3|1:1|3:4|9:16|21:9|adaptive'(optional, provider-specific), 'resolution': '480p|720p|1080p|4k'(optional, provider-specific), 'duration_seconds': '2-15 integer or -1 for supported Seedance models'(optional, provider-specific), 'generate_audio': true(optional, seedance 2.0 only), 'watermark': false(optional, seedance or dashscope), 'negative_prompt': 'things to avoid'(optional, veo or kling), 'person_generation': 'allow_all|allow_adult'(optional, veo only), 'seed': 123(optional), 'model_name': 'doubao-seedance-2-0-260128|doubao-seedance-2-0-fast-260128|kling-v3|wan2.7-t2v|wan2.7-t2v-2026-04-25|happyhorse-1.0-t2v'(optional, provider-specific), 'kling_mode': 'std|pro'(optional, kling only), 'prompt_extend': true(optional, dashscope Wan only)}",
  "{'input_path': 'workspace/path.png', 'image_url': 'https://example.com/source.png'(optional, dashscope happyhorse i2v requires URL), 'prompt': 'animate this image'(optional), 'prompt_rewrite': 'auto|off'(optional, agent-side), 'provider': 'seedance|veo|kling|dashscope'(optional), 'mode': 'first_frame', 'aspect_ratio': '16:9|4:3|1:1|3:4|9:16|adaptive'(optional, provider-specific), 'resolution': '480p|720p|1080p|4k'(optional, provider-specific), 'duration_seconds': '2-15 integer or -1 for supported Seedance models'(optional, provider-specific), 'generate_audio': true(optional, seedance 2.0 only), 'model_name': 'doubao-seedance-2-0-260128|doubao-seedance-2-0-fast-260128|kling-v3|wan2.7-i2v|wan2.7-i2v-2026-04-25|happyhorse-1.0-i2v'(optional, provider-specific), 'kling_mode': 'std|pro'(optional, kling only), 'prompt_extend': true(optional, dashscope Wan only)}",
  "{'input_paths': ['workspace/first.png', 'workspace/last.png'], 'prompt': 'transition between them'(optional), 'prompt_rewrite': 'auto|off'(optional, agent-side), 'provider': 'seedance|veo|kling|dashscope'(optional), 'mode': 'first_frame_and_last_frame', 'aspect_ratio': '16:9|4:3|1:1|3:4|9:16|adaptive'(optional, provider-specific), 'resolution': '480p|720p|1080p|4k'(optional, provider-specific), 'duration_seconds': '2-15 integer or -1 for supported Seedance models'(optional, provider-specific), 'generate_audio': true(optional, seedance 2.0 only), 'model_name': 'doubao-seedance-2-0-260128|doubao-seedance-2-0-fast-260128|kling-v3|wan2.7-i2v|wan2.7-i2v-2026-04-25'(optional, provider-specific), 'kling_mode': 'std|pro'(optional, kling only), 'prompt_extend': true(optional, dashscope Wan only)}",
...
- Use `dashscope` when the user asks for Aliyun Model Studio, Wan 2.7, or HappyHorse 1.0 video generation. Current DashScope support is limited to text-to-video, first-frame image-to-video, and first/last-frame image-to-video; do not route video editing or reference-video requests here yet.
...
- `dashscope` supports `wan2.7-t2v`, `wan2.7-t2v-2026-04-25`, `happyhorse-1.0-t2v`, `wan2.7-i2v`, `wan2.7-i2v-2026-04-25`, and `happyhorse-1.0-i2v`. `happyhorse-1.0-i2v` requires `image_url` or `image_urls`; Wan 2.7 image-guided routes can use local workspace images.
