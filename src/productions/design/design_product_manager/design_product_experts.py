"""Private expert allowlist for DesignProductManager."""
...
DESIGN_PRODUCT_EXPERT_ALLOWLIST: tuple[str, ...] = (
...
def is_design_product_expert(agent_name: str) -> bool:
    """Return whether an expert is private to DesignProductManager."""
    return str(agent_name or "").strip() in DESIGN_PRODUCT_EXPERT_ALLOWLIST


def build_design_expert_listing(
    expert_agents: dict[str, Any] | None = None,
) -> list[dict[str, Any]]:
    """Return tool-visible metadata for DesignProductManager private experts."""
    available_agents = expert_agents or {}
    experts: list[dict[str, Any]] = []
    for agent_name in DESIGN_PRODUCT_EXPERT_ALLOWLIST:
