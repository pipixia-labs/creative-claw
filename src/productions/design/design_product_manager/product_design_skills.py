"""Private product-design skill discovery for DesignProductManager."""
...
PRODUCT_DESIGN_SKILLS_DIR = Path("skills/product-design-skills")


@dataclass(frozen=True, slots=True)
class ProductDesignSkillInfo:
    """Metadata for one private product-design skill."""
...
class ProductDesignSkillRegistry:
    """Discover and read DesignProductManager-private skills."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the registry under the project-private skill root."""
        self.project_root = Path(project_root or PROJECT_PATH).resolve()
        self.skills_dir = (
            Path(skills_dir).resolve()
            if skills_dir is not None
            else (self.project_root / PRODUCT_DESIGN_SKILLS_DIR).resolve()
        )

    def list_skills(self) -> list[ProductDesignSkillInfo]:
        """Return all private product-design skills."""
        if not self.skills_dir.exists() or not self.skills_dir.is_dir():
            return []

        skills: list[ProductDesignSkillInfo] = []
    ...
    def read_skill(self, name: str) -> str:
        """Read one private product-design skill by exact folder name."""
        ...
        raise ValueError(f"Product design skill '{skill_name}' not found.")
