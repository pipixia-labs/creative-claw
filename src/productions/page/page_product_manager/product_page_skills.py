"""Private product-page skill discovery for PageProductManager."""
...
PRODUCT_PAGE_SKILLS_DIR = Path("skills/product-page-skills")


@dataclass(frozen=True, slots=True)
class ProductPageSkillInfo:
    """Metadata for one private product-page skill."""
...
class ProductPageSkillRegistry:
    """Discover and read PageProductManager-private skills."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the registry under the project-private page skill root."""
        self.project_root = Path(project_root or PROJECT_PATH).resolve()
        self.skills_dir = (
            Path(skills_dir).resolve()
            if skills_dir is not None
            else (self.project_root / PRODUCT_PAGE_SKILLS_DIR).resolve()
        )

    def list_skills(self) -> list[ProductPageSkillInfo]:
        """Return all private product-page skills."""
        if not self.skills_dir.exists() or not self.skills_dir.is_dir():
            return []

        skills: list[ProductPageSkillInfo] = []
    ...
    def read_skill(self, name: str) -> str:
        """Read one private product-page skill by exact folder name."""
        ...
        raise ValueError(f"Product page skill '{skill_name}' not found.")
