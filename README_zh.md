...
## 📰 News
 - 2026-06-02：发布 v0.5；支持 Google ADK 2.1，完善 provider-aware 结构化最终响应、ADK-native PPT HITL 桥接、Product/Expert 协议 schema 化、OpenAI Codex OAuth 模型路径，并提升 Web Design/Page/PPT 交付稳定性。
 - 2026-05-22：发布 v0.4；升级 Web 视觉工作区，完善 PPT/Page/Design 产品流，增强 LLM 兼容性，优化视频预览、Activity 聚合和 runtime trace 调试能力。
 - 2026-05-10：新增 Web 设计任务的需求确认支持和 design system 支持。
 - 2026-05-09：升级 Design 与 Web Chat 工作流，新增基于 tldraw 的 Visual Board、类画布 Design 预览和路径化视觉附件。
 - 2026-05-05：新增交互式 PPT 生成与 HTML 转 PPTX 流程；优化 Web Chat 对 artifact、媒体、PDF、PPT 产物的预览体验。
 - 2026-05-02：发布 v0.3.0；支持 DeepSeek V4、Seedance 2.0、Seed TTS 2.0 多音色、DashScope 媒体模型，并扩展 3D provider。
 - 2026-05-01：新增设计生产流和 CodeGenerationExpert 路由。
 - 2026-04-24：发布 v0.2.0；重构 expert runtime，统一 expert cards，并去重基础媒体 agent。
 - 2026-04-21：接入 Kling 视频生成，支持文生视频、图生视频和多图参考流程。
 - 2026-04-14：支持混元 3D，并扩展内置媒体 expert 集合。
 - 2026-04-13：扩展 LLM provider 覆盖，并新增图像分割。
 - 2026-04-12：发布 v0.1.1；支持 Web、CLI、飞书中的核心图像/视频流程。
...
- **Web Chat 视觉工作区**：可以在 Design 预览中检查生成结果，在 Visual Board 上标注图像产物，并把画布选区作为 workspace 文件附件发回对话。
- **设计需求表与设计系统**：Web 设计任务可以先通过紧凑需求表确认关键信息，从内置 design system 目录推荐合适风格，预览明暗模式，并把选中的设计系统带入后续生成。
...
### 🧠 LLM
 - `openai`、`openai_codex`（配置里也支持写 `openai-codex`）、`anthropic`、`gemini`、`openrouter`、`deepseek`、`groq`、`zhipu`、`dashscope`、`vllm`、`ollama`、`moonshot`、`minimax`、`mistral`、`stepfun`、`siliconflow`、`volcengine`、`byteplus`、`qianfan`、`azure_openai`、`custom`
 - `openai_codex` / `openai-codex` 通过 ChatGPT/Codex OAuth 登录使用，不需要 `OPENAI_API_KEY`。使用前先运行 `creative-claw provider login openai-codex` 完成认证，然后配置 `llm.provider` 为 `openai-codex` 或 `openai_codex`，并设置 `llm.model="gpt-5.5"`。
...
### 🖼️ 图像生成
 ...
 - DashScope 图像模型（`wan2.7-image-pro`、`qwen-image-2.0-pro`、`z-image-turbo`）
### 🎬 视频生成
...
推荐使用 Python 3.13 作为运行环境。Python 3.12 也支持，但暂不建议使用 Python 3.14，因为 Seedance / Seedream 走的 Volcengine Ark SDK 目前依赖 Pydantic v1 兼容层，在 Python 3.14 下不稳定，可能出现 warning 或真实调用失败。
...
- 这已经足够体验默认的 CLI 聊天流程。
- 如果要用 Codex OAuth 账号而不是 OpenAI API key，先运行 `creative-claw provider login openai-codex` 完成认证，然后把 `llm.provider` 设为 `openai-codex` 或 `openai_codex`、`llm.model` 设为 `gpt-5.5`；该路线不需要 `providers.openai.api_key`。
...
- Seedance / Volcengine Ark 调用当前建议运行在 Python 3.13。Python 3.14 可能触发 Ark SDK 的 Pydantic v1 兼容层 warning 或调用失败。
...
- DashScope 媒体 provider 使用 `providers.dashscope.api_key` 或 `DASHSCOPE_API_KEY`。`VideoGenerationAgent` 当前只接入 DashScope 文生视频、首帧图生视频、首尾帧图生视频；视频编辑和参考视频路线本轮先不暴露。
- `ImageGenerationAgent` 的 `provider="dashscope"` 支持 `model_name="wan2.7-image-pro"`、`model_name="qwen-image-2.0-pro"` 和 `model_name="z-image-turbo"`。
...
- 第一轮文本 LLM provider 已支持：`openai`、`openai_codex`、`anthropic`、`gemini`、`openrouter`、`deepseek`、`groq`、`zhipu`、`dashscope`、`vllm`、`ollama`、`moonshot`、`minimax`、`mistral`、`stepfun`、`siliconflow`、`volcengine`、`byteplus`、`qianfan`、`azure_openai`、`custom`。
...
多数创意工作流推荐优先使用 Web Chat，因为它对进度和产物预览做了专门优化：
...
如果你已经执行过 `pip install -e .`，也可以直接使用 CLI Chat：
...
- Seedance 使用 Volcengine Ark，当前已用 Python 3.13 验证。Ark SDK 完整支持 Python 3.14 之前，这条路径不要使用 Python 3.14。
...
- `dashscope`：模式包括 `prompt`、`first_frame`、`first_frame_and_last_frame`；文生视频模型包括 `wan2.7-t2v`、`wan2.7-t2v-2026-04-25`、`happyhorse-1.0-t2v`；图生视频模型包括 `wan2.7-i2v`、`wan2.7-i2v-2026-04-25`、`happyhorse-1.0-i2v`。HappyHorse 图生视频需要 `image_url` / `image_urls`；Wan 2.7 图生视频可以使用本地 `input_path` / `input_paths`。

`ImageGenerationAgent` 当前支持这些 provider-aware 参数：

- 通用控制项：`provider`、`prompt`、`aspect_ratio`、`resolution`、`size`、`model_name`、`negative_prompt`、`prompt_extend`、`watermark`、`thinking_mode`。
- `nano_banana`：默认 Gemini 文生图 provider；支持 `aspect_ratio` 和 `resolution`。
- `seedream`：火山 Seedream 文生图 provider。
- `gpt_image`：OpenAI GPT Image provider；支持 `size` 和 `quality`。
- `dashscope`：阿里云百炼 / Model Studio provider；支持模型 `wan2.7-image-pro`、`qwen-image-2.0-pro`、`z-image-turbo`。
...
- **本地 Web Chat（推荐）**：浏览器里聊天，针对实时进度、Markdown、紧凑设计需求表、design system 预览、Design 预览、Visual Board 标注，以及媒体 / PDF / PPT 预览做了专门优化
- **CLI Chat**：轻量命令行入口，适合快速调用和自动化
