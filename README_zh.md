...
## 📰 News
 - 2026-05-09：升级 Design 与 Web Chat 工作流；新增 DesignProductManager 私有 `DesignCodeGenerationAgent`、Claude Design 风格画布生成提示、类画布的 Design 预览缩放/平移，以及基于 tldraw 的 Visual Board 图像标注与选区发送到对话。
 - 2026-05-09：Web Chat 图像附件改为路径优先的工作流元数据；Visual Board 导出的选区会作为 workspace 文件路径传递，避免把图片以内联/base64 形式塞进主模型上下文。
...
- **本地 Web Chat（推荐）**：浏览器里聊天，针对实时进度、Markdown、Design 预览、Visual Board 标注，以及媒体 / PDF / PPT 预览做了专门优化
